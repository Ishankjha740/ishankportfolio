    ...
    <DialogPrimitive.Root open={open} onOpenChange={onOpenChange}>
      <DialogPrimitive.Portal>
        ...
        >
          ...
          <div className="flex items-center justify-between gap-4 border-b-2 border-paper px-5 sm:px-8 py-4 sm:py-5 bg-ink sticky top-0 z-20">
            <div>
              <DialogPrimitive.Title asChild>
                <h2 className="display-heading text-xl sm:text-2xl md:text-3xl text-paper leading-none">
                  Let's <span className="bg-citrus px-2 text-ink">Connect</span>
                </h2>
              </DialogPrimitive.Title>
              <DialogPrimitive.Description className="mt-2 text-[10px] sm:text-xs uppercase tracking-[0.25em] font-bold text-paper/60">
                Drop the details — I'll be in touch
              </DialogPrimitive.Description>
            </div>
            <DialogPrimitive.Close
              aria-label="Close"
              className="shrink-0 inline-flex items-center justify-center w-10 h-10 sm:w-11 sm:h-11 border-2 border-paper text-paper hover:bg-citrus hover:text-ink hover:border-citrus transition-colors duration-200"
            >
              <X size={18} strokeWidth={2.5} />
            </DialogPrimitive.Close>
          </div>

          <div className="p-3 sm:p-5 md:p-6 bg-ink">
